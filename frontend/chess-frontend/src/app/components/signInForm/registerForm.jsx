...
import { updateField, setErrors, registerUser } from '../../store/authSlice/registerFormSlice';
import {
  Container,
  Box,
  Typography,
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      try {
        ...
        // Sprawdzenie, czy 'error' jest bezpośrednio tablicą lub czy data odpowiedzi jest tablicą
        ...
        if (errorsArray) {
          errorsArray.forEach((err) => {
            if (err.code === 'DuplicateUserName') {
              serverErrors.username = err.description;
            } else if (err.code === 'DuplicateEmail') {
              serverErrors.email = err.description;
            } else if (err.code.startsWith('Password')) {
              serverErrors.password = err.description;
              serverErrors.confirmPassword = err.description;
            }
          });
        } else {
          serverErrors.general = 'An unexpected error occurred.';
        }
